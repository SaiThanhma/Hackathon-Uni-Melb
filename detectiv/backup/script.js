...
function newGame() {
  ...
  document.getElementById('normal-input-area').style.display = '';
...
function openAccuseTrial(suspectName, deadlineMs) {
  ...
  // Show accusation panel, hide normal input
  document.getElementById('accuse-panel-trial').style.display = 'flex';
  document.getElementById('normal-input-area').style.display = 'none';
  // Switch to the accused suspect's tab so hearing is contextual
...
async function sendAccusationTrialMessage() {
  ...
  try {
    ...
    renderAccuseTrialLine('suspect', data.reply);
    if (data.won) {
      clearAccusationTimer();
      setAccuseTrialInputEnabled(false);
      await finalizeAccusationOutcome({
        ...
        confession_reply: data.confession_reply,
        abandoned: false,
      });
    }
  } catch (e) {
...
async function finalizeAccusationOutcome(data) {
  // Hide accusation panel, restore normal input area
  document.getElementById('accuse-panel-trial').style.display = 'none';
  document.getElementById('normal-input-area').style.display = '';
  clearAccusationTimer();
  ...
  // Replay accusation dialogue into the main chat transcript
  if (accusationTargetName) {
    switchSuspect(accusationTargetName);
    addMsg(accusationTargetName, 'system', '— Formal accusation hearing —', 'talk');
    accusationTrialLines.forEach((line) => {
      const role = line.role === 'player' ? 'player' : 'suspect';
      addMsg(accusationTargetName, role, line.text, 'accuse');
    });
  }

  // If won: type the confession fully, then show solved overlay
  if (data.won && data.confession_reply) {
    await addMsgTyped(accusationTargetName, 'suspect', data.confession_reply, 'confess');
  }

  // Only now show the solved overlay
